...
namespace CoreBanking.Domain.Entities
{
    public class Account
    {
        ...
        //navigate properties - private to enforce aggregate boundary
        ...
        //Core banking operations - these are the aggregate's public APU
        public Transaction Deposit(Money amount, string description = "Deposit")
        {
            if (!IsActive)
                throw new InvalidOperationException("Connot deposit in an inactive account");

            if (amount.Amount <= 0)
                throw new InvalidOperationException("Deposit amount be positive");
        ...
        {
            if (!IsActive)
                throw new InvalidOperationException("Cannoth withdraw from inactive account");
